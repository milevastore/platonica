...
var simpleStore = {
    ...
    settings: {
        ...
        notifyContainer: $('.simpleStore_notify_container'),
	...
	checkout : function (s, checkoutData) {
        if (this.settings.checkoutDisabled){
            simpleStore.notifier('Se encuentra deshabilitado el checkout.');
        }
		else if (!$.isEmptyObject(checkoutData) && !this.settings.checkoutDisabled) {
        	simpleCart.checkout();
    ...
    validatePrices : function (s) {
        var checkoutData = JSON.parse(localStorage.simpleCart_items),
			errorMsg = 'Ocurrio un error validando tu carrito de compras.';
    ...
    generateStore: function () {
		...
		simpleCart({
			afterAdd: function() {
				simpleStore.notifier('Se agrego el producto a tu carrito de compras.');
			},
